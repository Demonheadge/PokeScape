...
# https://github.com/pret/pokeemerald/pull/2068/files
ifdef CLION_IDE
...
CPPFLAGS := $(INCLUDE_CPP_ARGS) -iquote $(GFLIB_SUBDIR) -Wno-trigraphs -DMODERN=1 -DTESTING=$(TEST)
ARMCC := $(PREFIX)gcc
...
RULES_NO_SCAN += libagbsyscall clean clean-assets tidy tidymodern tidycheck generated clean-generated emerald clean-emerald
.PHONY: all rom agbcc modern compare check debug patch
.PHONY: $(RULES_NO_SCAN)
...
# TODO: gflib files in pokeemerald-expansion are in `include` and `src` directories, these should eventually be moved to match so we can remove these extra gflib calls
...
clean: tidy clean-tools clean-check-tools clean-generated clean-assets clean-emerald
...
emerald:
	@$(MAKE) -C $(TOOLS_DIR)/pokeemerald MODERN=1
	cp $(TOOLS_DIR)/pokeemerald/pokeemerald*.gba ./pokeemerald.gba

patch: all emerald
	$(FLIPS) --manifest=patch.xml pokeemerald.gba $(ROM_NAME) $(PATCH_NAME)
...
# TODO: Refactor to mimic pokeemerald-expansion if possible
...
# Calling `$(PREPROC) $< charmap.txt -i` HAS to have `-i` AFTER charmap.txt, and NOT after `$(PREPROC)`, e.g. `$(PREPROC) -i $< charmap.txt` causes compilation failure
# Probably due to `tools/preproc` being quite old compared to the one in `pokeemerald-expansion`
...
$(C_BUILDDIR)/%.d: $(C_SUBDIR)/%.c
	$(SCANINC) -M $@ $(INCLUDE_SCANINC_ARGS) -I tools/agbcc/include -I $(GFLIB_SUBDIR) $<
...
$(GFLIB_BUILDDIR)/%.d: $(GFLIB_SUBDIR)/%.c
	$(SCANINC) -M $@ $(INCLUDE_SCANINC_ARGS) -I tools/agbcc/include -I $(GFLIB_SUBDIR) $<

ifneq ($(NODEP),1)
-include $(addprefix $(OBJ_DIR)/,$(GFLIB_SRCS:.c=.d))
endif
...
# NOTE: In pokeemerald-expansion, the line is:
...
# .s files, e.g. $(DATA_ASM_BUILDDIR)/%.o: $(DATA_ASM_SUBDIR)/%.s
# NOTE: In pokeemerald-expansion, the line is:
#   $(PREPROC) $< charmap.txt | $(CPP) $(INCLUDE_SCANINC_ARGS) - | $(PREPROC) -ie $< charmap.txt | $(AS) $(ASFLAGS) -o $@
# Probably due to `tools/preproc` being quite old compared to the one in `pokeemerald-expansion`
